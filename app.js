...
class OptionValue {
    constructor(name = "", value = "", flags = "", description = "") {
        this.name = name;
        this.value = value;
        this.flags = flags;
...
class Option {
    constructor(name = "", type = "", flags = "", description = "") {
        this.name = name;
        this.type = type;
        this.flags = flags;
        this.description = description;
...
class Encoder {
    constructor(name = "", description = "", generalCapabilities = "", threading = "", flags) {
        this.name = name;
        this.description = description;
...
class Protocol {
    constructor(name = "", direction = "", fullHelp) {
        this.name = name;
...
class Format {
    constructor(flags = "", name = "", description = "", canDemux = false, canMux = false, isDevice = false, fullHelp) {
        this.flags = flags;
        this.name = name;
        this.description = description;
...
class Filter {
    constructor(flags = "", name = "", description = "", fullHelp) {
        this.flags = flags;
        this.name = name;
        this.description = description;
        this.fullHelp = fullHelp;
...
async function runFFmpeg(args) {
    try {
        const { stdout } = await execAsync(`ffmpeg -hide_banner -loglevel error ${args}`, { maxBuffer: 1024 * 1024 * 10 });
        return stdout;
    }
    catch (err) {
...
async function runLimitedParallel(items, limit, workerFn) {
    const results = [];
    let index = 0;
...
// Parse section list
// -------------------------
function parseFlagsNameDesc(output, sectionHeading) {
...
function parseFormats(output) {
    ...
    for (const line of lines) {
        ...
        items.push(new Format(flags.trim(), name.trim(), description.trim(), flags.includes("D"), flags.includes("E"), flags.includes("d")));
    }
    return items;
}
// -------------------------
...
// -------------------------
async function fetchHelp(entryName) {
    let h = await runFFmpeg(`-h ${entryName}`);
    h = h.replace(/Exiting with exit code 0/g, "");
    return h.trim();
}
async function fetchHelpForFormat(item, type) {
...
function parseEncoderHelp(helpText) {
    ...
    for (const line of lines) {
        ...
        // AVOptions
        const optMatch = trimmed.match(/^(-\S+)\s+<([^>]+)>\s+([A-Z\.]+)\s+(.+?)(?:\(default\s*(.+?)\))?$/);
        if (optMatch) {
            if (currentOption)
                encoder.options.push(currentOption);
        ...
        if (currentOption && line.startsWith("     ") && enumMatch) {
            ...
            currentOption.values.push(new OptionValue(name, parseFloat(value) || value, flags, desc.trim()));
            continue;
        }
...
// -------------------------
function parseProtocolHelp(helpText) {
    const lines = helpText.split(/\r?\n/);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (optMatch) {
            if (currentOption)
                protocol.options.push(currentOption);
        ...
        if (currentOption && line.startsWith("     ") && !line.match(/^\s+\w+\s+</)) {
            ...
            if (valMatch) {
                currentOption.values.push(new OptionValue(valMatch[1], isNaN(Number(valMatch[2])) ? valMatch[2] : parseInt(valMatch[2]), "", valMatch[3].trim()));
            }
            continue;
        }
    }
...
// -------------------------
function parseFormatHelp(helpText) {
    const lines = helpText.split(/\r?\n/);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (optMatch) {
            if (currentOption)
                format.options.push(currentOption);
            const [, name, type, flags, desc] = optMatch;
            const defaultMatch = desc.match(/\(.*?default\s+([^)]+)\)/);
            const rangeMatch = desc.match(/\(from\s+(-?[\w.]+)\s+to\s+(-?[\w.]+)\)/);
            currentOption = new Option(name.trim(), `<${type}>`, flags.trim(), desc.trim());
        ...
        if (currentOption && line.startsWith("     ") && !line.match(/^\s+-\w+\s+</)) {
            ...
            if (valMatch) {
                ...
                currentOption.values.push(new OptionValue(name, name, flags, description));
            }
            continue;
        }
    }
...
// -------------------------
function parseFilterHelp(helpText, filter) {
    const lines = helpText.split(/\r?\n/);
    // const filter: Filter = {
    //   flags: "",
    //   name: "",
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentSection === "options") {
            ...
            if (optMatch) {
                if (currentOption)
                    filter.options.push(currentOption);
                const [, name, type, flags, desc] = optMatch;
                const defaultMatch = desc.match(/\(.*?default\s+([^)]+)\)/);
                const rangeMatch = desc.match(/\(from\s+(-?[\w.]+)\s+to\s+(-?[\w.]+)\)/);
            ...
            if (currentOption && line.startsWith("     ") && !line.match(/^\s+\w+\s+</)) {
                const valMatch = line.trim().match(/^(\w+)\s+(\S+)\s+(.*)$/);
                if (valMatch) {
                    currentOption.values.push(new OptionValue(valMatch[1], valMatch[2], "", valMatch[3].trim()));
                }
                continue;
            }
        }
...
async function getFFmpegCapabilities() {
    ...
    const [protocolsRaw, demuxersRaw, muxersRaw, encodersRaw, filtersRaw] = await Promise.all([
    ...
    if (!skipSet.has("--skip-encoders")) {
        const encoders = parseFlagsNameDesc(encodersRaw, "------").slice(skip, skip ? skip + (limit || Infinity) : limit);
        console.log(`Fetching help for ${encoders.length} encoders...`);
        const encoderHelp = await runLimitedParallel(encoders, 100, async (p) => {
    ...
    if (!skipSet.has("--skip-filters")) {
        const filters = parseFlagsNameDesc(filtersRaw, "| = Source or sink filter").slice(skip, skip ? skip + (limit || Infinity) : limit);
        console.log(`Fetching help for ${filters.length} filters...`);
        const filterHelp = await runLimitedParallel(filters, 100, async (p) => {
...
getFFmpegCapabilities().then((json) => {
    ...
    (0, fs_1.writeFileSync)("C:\\dev\\node-ffmpeg-helper\\example_output.json", JSON.stringify(json, null, 2));
});
